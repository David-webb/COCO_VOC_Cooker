 ...
 * Created time  : 2021-04-13 11:32
 * Filename      : voc2coco.py
 * Description   :
"""
# -*- coding=utf-8 -*-
#!/usr/bin/python
...
PRE_DEFINE_CATEGORIES = {"__background__":0}
...
if __name__ == '__main__':
    root_path = "./VOC_extracter/VOC2012/"
    this_dir = os.getcwd()
    print(this_dir)
    save_dir = os.path.join(this_dir, 'voc2coco', 'all_fms_imgs')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    xml_dir = os.path.join(root_path, 'Annotations_ssd')
    xml_list = os.listdir(xml_dir)
    json_file = './voc2coco/annotations_all.json' # person_keypoints_train2017.json / person_keypoints_val2017.json
    convert(xml_list, xml_dir, json_file)
    for xml_file in xml_list:
        img_name = xml_file[:-4] + '.jpg'
        shutil.copy(os.path.join(root_path, 'JPEGImages_ssd', img_name),
                    os.path.join(save_dir, img_name))

    # =======================以下是demo ===================================
    # root_path = os.getcwd() # 获得当前的工作目录
